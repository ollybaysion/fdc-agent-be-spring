...
가져오고, 수집 툴(`retrieve_data`·`request_input`)은 실행하지 않고 "이게 필요하다"를
...
**① 요청은 풀 안에서만.** `QueryPool` 은 로드된 스킬 spec 의 `queries[]` 를 평면화한
목록이고, `retrieve_data` 의 `skill` 은 그 목록의 **닫힌 enum** 이다. 모델이 대는 것은
절차와 인자뿐이고 **무엇을 조회할지는 판정이 정한다**(단위가 쿼리가 아니라 need 다) —
실행 문장(`SqlRender`)·조회 키(`QueryKey`)·기대 컬럼은 BE 가 만든다. 렌더된 SQL 은
**사용자가 자기 권한으로 실행**하므로 이스케이프는 미관이 아니라 경계다. 같은 spec 이
조회 툴로도 컴파일되므로, 스킬을 등재하면 실행과 조달이 함께 늘고 등재되지 않은 조회는
어느 쪽으로도 나가지 않는다. 풀이 비면 `retrieve_data` 자체가 안 붙는다.

**② 진행은 저장하지 않고 도착에서 유도한다.** `queryKey = {스킬}#{조달id}__{필수인자}` 라
한 절차의 모든 조달이 같은 run 이름표를 단다. 이번 요청에 실려 온 스냅샷의 키만 풀에
비추면 "무엇을 알아냈고 무엇이 모자란지"가 나오고(`QueryProgress`), 그 한 걸음이 맥락에
실려 모델을 민다. 계약에 진행 필드도, 서버 세션도 필요 없다. 앞 조달 결과를 바인드로
쓰는 조달은 **도착한 표에서 값을 읽는다** — 여러 값이면 지금은 고르지 않고 잠긴 것으로
보고한다(갈림길의 결정 주체가 미정, #50).

**②-1 채움은 3차 폭포다.** need 하나마다 위에서 걸리면 끝난다 — ① 지목한 조달의 키로
정확히 도착 ② **같은 대상의 다른 표**에 그 컬럼이 있음(`ArrivalLens`, 결정론) ③ 이름이
전혀 다른 표를 **LLM 이 대조**(`AltFillJudge`). 3차는 못 채운 need 와 결정론이 못 쓴 표가
동시에 있을 때만 부르고, 모델이 고른 값이 그 표에 실제로 없으면 버린다. 어느 경로로
찼는지는 `runsProgress[].needs[].source` 에 남는다 — 그 값이 다음 조회의 바인드로
들어가므로 출처가 추적 가능해야 한다.
...
- `POST /api/fdc/v1/chat/data` — 데이터 패널 판정 인렛(panel-judge, #38). 패널의
  수정·입력마다 호출되고 `PanelJudge` 가 무상태 결정론으로 진행·종결을 판정한다.
  응답에는 **조달 원장**(`dataRequests`)이 함께 나간다: 그 절차의 조달 전량이
  상태(`ready`·`blocked`·`arrived`·`inactive`·`unreachable`)를 달고 매번 전부
  실리고 FE 는 replace 한다. 무엇을 보일지 판단하는 것도, 스킬을 읽어 SQL 을
  완성하는 것도 화면 쪽 일이 아니다. 절차 종결 전이만 그 응답의 SSE
  `token*` 으로 최종 서술(LLM 은 서술과 3차 대조에만 남는다). 대화 인렛은 전부
  `/chat/*` 네임스페이스다.
