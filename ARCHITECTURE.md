...
| 빌드 | **Gradle 8** (wrapper) | 레포 자체 완결 재현 빌드(`./gradlew`), JDK 21 toolchain |
...
| 로깅 | **SLF4J/Logback + MDC** | `X-Request-Id` 전파, 민감 헤더 비기록 |
| 테스트 | **JUnit 5 + Spring Boot Test/MockMvc** | vitest 스위트 1:1 포팅(25 테스트). MockMvc = Fastify `.inject()` 대응 |
| 회귀 판정 | **패리티 하네스** (Node 판 병행 기동 + diff) | deprecated Node 판의 마지막 역할 — 24케이스 byte-identical 기준선 |
...
단일 Gradle 모듈. Spring 이 관여하는 곳은 **config/·web/ 두 패키지뿐** —
나머지는 프레임워크 중립 코어라 컨테이너 없이 `new` 로 조립·테스트한다.
...
└── src/
    ...
    │   │              # RequestIdFilter · ApiExceptionHandler
    ...
    │   ├── llm/       # LlmClient seam: MockLlm ↔ OpenAiLlm
    ...
    └── test/java/fdc/agent/           # JUnit 25 (vitest 1:1 포팅)
```
...
- 2 컨테이너(FE·BE) — docker-compose(로컬)/k8s(운영). 내부 DNS로 프록시.
- 브라우저는 FE와만 통신(same-origin) → **CORS 불필요**.
- BE만 Oracle·LLM 도달. 브라우저는 둘 다 못 봄(반출 차단 자연 충족).
- 런타임 이미지 = `eclipse-temurin:21-jre` 계열.
...
2. ~~온프렘 모델 tool calling 지원 여부~~ — **해소** (실 Claude 로 툴 루프 end-to-end
   입증, 사내 GW 는 env 3개만 교체).
3. **Oracle 스키마 매핑 = 사내 남음** — 계약 각 필드가 어느 테이블/컬럼인지.
   agent-db-plugin(describe_table)으로 실 스키마를 떠서 `SchemaMap.java` 한 파일만
   채우면 됨(절차 = Node 레포 `docs/phase1-사내-runbook.md`, .ts→.java 로 읽으면 동일).
...
| — | **Spring 전체 포팅** (패리티 24 byte-identical + JUnit 25) | ✓ | ✓ | **완료** — 이후 기준 구현 |
...
- 서버측 입력 캡 재적용(messages≤100, content≤10k, context≤50) → `messages_too_many` 등.
- production 에서 SSE `error.message`·5xx body 에 stack/내부경로/DB메시지 금지, 상세는 로그만.
- 모든 응답 `X-Request-Id`(+`X-Fdc-Data-Source`), 민감 헤더 비기록.
- 응답측 캡(rows≤1000, chart points≤5000, timeline≤500, 누적 chars≤100k) → `truncated`/`length`.
